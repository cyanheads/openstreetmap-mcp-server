...
# ==============================================================================
# `--platform=$BUILDPLATFORM` keeps this stage on the builder's native architecture.
# Under QEMU emulation Bun 1.4.0 aborts in JavaScriptCore (exit 134), so a multi-arch
# publish from an arm64 host fails its linux/amd64 leg deterministically without it.
# The stage only compiles TypeScript, so its output is architecture-independent.
...
# `--omit=peer` drops the framework's optional peer tiers (test runner, service SDKs,
# parsers) that Bun would otherwise auto-install. Anything this server actually imports
# belongs in its own `dependencies`, so nothing needed at runtime is lost. The OTEL step
# below carries the same flag — without it, that install re-resolves the graph and pulls
# every optional peer back in.
RUN --mount=type=cache,target=/root/.bun/install/cache \
