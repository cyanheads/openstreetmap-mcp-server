...
| `openstreetmap_query_raw` | Execute a raw Overpass QL query for advanced spatial queries the convenience tools don't cover. | `query` (Overpass QL string), `limit`, `offset`, `timeout_seconds` | `readOnlyHint: true` |
...
```ts
z.object({
  elements: z.array(z.record(z.unknown())).describe('Raw Overpass API response elements for this page, up to the limit. Structure varies by query type — nodes have lat/lon, ways have nodes[], relations have members[].'),
